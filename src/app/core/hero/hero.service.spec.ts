...
describe('HeroService', () => {
  ...
  it('should add a new hero', async () => {
    ...
    expect(service.getHeroes()[0].name).toBe('Superman');
  });

  it('should delete a hero by id', async () => {
    await service.addHero('Batman');
    const heroId = service.getHeroes()[0].id;
  ...
  it('should edit a hero name', async () => {
    ...
    expect(service.getHeroes()[0].name).toBe('Green Lantern');
  });
  ...
  it('should filter heroes by name', async () => {
    ...
    expect(service.getHeroes().length).toBe(1);
    expect(service.getHeroes()[0].name).toBe('Hulk');
  });
